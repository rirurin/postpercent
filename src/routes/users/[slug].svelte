...
<header style="background-color:{headColor}; color:{lightText == 1 ? `var(--background)` : `var(--text)`}" class="main-header">
...
<header style="background-color:{headColor}; color:{lightText == 1 ? `var(--background)` : `var(--text)`}" class="sticky-header">
	<li class="sticky-pfp">
		{#if user}
		<a href="/users/{user.username}"><img src="https://cdn2.scratch.mit.edu/get_image/user/{user.id}_150x150.png" alt={slug}></a>
		{:else}
